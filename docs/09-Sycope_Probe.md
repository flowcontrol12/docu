...
​	`Sycope-Probe(config)# filters regex-list myList pattern /CGS*Tower*/i`
...
| 2        | Any         | Regular expression: /CGS*/i | Session tracking,<br />Timestamping,<br />Redirect to Port 2 |


System operation is as follows:

1.  Packets with VLAN 4000 are dropped (Filter 1).
1.  Packets matching the regular expression /CGS\*/i create an active tracked session, a timestamp is added to them, and they are redirected to Port 2 (Filter 2).
...
​	`Sycope-Probe host1/127.0.0.1;session 12345678; 1.2.3.4:1000-4.3.2.1:2000 TCP;matched by filter 1;app=dns;app-attributes= [dns:qtype=1][dns:qclass=1][dns:qname=www.cgstowernetworks.com.cdn.cloudflare.net]`
...
​	`Sycope-Probe(config)# dns url-list myList pattern /CGS*Tower*.com/i`
...
To use IPFIX, it is required to install an activation license. Contact support@cgstowernetworks for more info.
...
This section describes the Sycope-Probe support of SNMP (Simple Network Management Protocol). The Sycope-Probe SNMP agent supports SNMP v2c read and write communities, SNMP v3 users, and acts as an SNMP trap sender. This section also provides a description of CGS proprietary MIB supported by the Sycope-Probe device.
...
1.3.6.1.4.1.47477.100.3: so(1).org(3).dod(6).internet(1).private(4).enterprises(1).cgs(47477).npb(100).Sycope-Probe(3)
...
| CGS-TRAPS   | Sycope-Probe traps configuration              |                       |
