...
| **limit**    | limit [{offset},] {limit} by {fieldName} | Truncates the output collection to a specified number of objects. Returns the first {limit} starting from position {offset}. Optional parameter "by" is used to search unique fields in the defined limit. | src stream="netflow" \| limit 1 by clientIp                              |
